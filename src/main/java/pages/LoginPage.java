...
//-----------------------------------------------------------------------------------------------------------//
	public void Login_test_with_Invalid_data(String email, String password) {
		try {
			...
			ActionUtils.Sendkeys(Email, email);

			ActionUtils.Sendkeys(Password, password);
...
//-----------------------------------------------------------------------------------------------------------//
	...
	public void Login_test_with_valid_data(String email, String password) throws InterruptedException {
		try {
			...
			ActionUtils.Clear_And_SendKeys(Email, email);
			
			ActionUtils.Clear_And_SendKeys(Password, password);

			ActionUtils.Click(Loginbtn);

			WaitUtils.Hard_Wait(3000);
			
			Assert.assertTrue(CheckUtils.Is_Displayed(Dashboard), "Dashboard is not displayed");
